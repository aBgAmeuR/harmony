...
const results = await Promise.all(
  [...albums].map((value) =>
    limit(async () => {
      ...
      const rg = await retryWithBackoff(() => mbApi.releaseGroups.search({ artist: artist, release: album }))

      if (rg['release-groups'].length <= 0) {
        const rg2 = await retryWithBackoff(() => mbApi.releaseGroups.search({ artist: artist, release: album }, false))
