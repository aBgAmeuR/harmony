...
export const activeDaysFn = async (params: ListeningRangeParams): Promise<ListeningHabitMetric> => {
  ...
  const [valueRows, trendRows] = await Promise.all([
    db.query<ListeningHabitValueRow>(`
      SELECT COUNT(DISTINCT CAST(i.ts AS DATE))::INTEGER AS value
      FROM interactions i
      ${artistJoin}
      ${where}
    `),
    db.query<ListeningHabitTrendRow>(`
      ...
      stats AS (
        SELECT
          ...
          COUNT(DISTINCT CAST(i.ts AS DATE))::INTEGER AS value
        FROM interactions i
        ${artistJoin}
        ${where}
        GROUP BY ${periodExpr}
